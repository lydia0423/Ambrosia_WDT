...
if(!isset($_SESSION["Theme"])) {
	...
	if (isset($_GET["TagID"])) {
		$TagID = $_GET["TagID"];
		$sqlRow = "SELECT COUNT(RecipeID) FROM recipes WHERE Tags = '$TagID'";
	} else {
	...
	if (isset($_GET["TagID"])) {
		$TagID = $_GET["TagID"];
		$sqlLoadRecipe = "SELECT * FROM recipes WHERE Tags = '$TagID' LIMIT $Offset, $PostsPerPage";
	} else {		
...
<body id="BrowseRecipeBody" class="GridlessBody">
<?php include("Header.php"); ?>
	...
	<!-- Content --> 
		<div class="browse-content clearfix">
			...
			<h2 id="Msg">There are no recipes under this tag!</h2>
<?php 
	...
	while ($Post = mysqli_fetch_assoc($result)) {
		...
		$Author = $Post["Author"];
		include("Scripts/Script_RecipeBrowsePosts.php");
			...
			<div class="sidebar">
				<div class="section search">
					...
					<!-- <form action="index.html" method="post"> -->
						...
						autoComplete="off">
						<!-- onblur="setTimeout..." => delays the hiding of the div by 100ms so that the redirect can occur -->
					<!-- </form> -->
				</div>
